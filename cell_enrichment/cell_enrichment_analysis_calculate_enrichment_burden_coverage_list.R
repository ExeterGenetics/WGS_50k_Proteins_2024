...
for(panel in c(levels(as.factor(panel_data$Panel)),"all")){
    ...
    for(i in seq(1,100)){
        #simulations_summary <- vroom(Sys.glob(paste0("/slade/projects/UKBB/DNA_Nexus_500k_WGS/Proteomics_50k_3k/results_summaries/rob_working_area/enrichment_sims/grouped/burden_enrichment_simulations_",panel,"_",i))) %>% bind_rows(simulations_summary)
        simulations_summary <- vroom(Sys.glob(paste0("/slade/projects/UKBB/DNA_Nexus_500k_WGS/Proteomics_50k_3k/results_summaries/rob_working_area/enrichment_sims/coverage_list/burden_enrichment_simulations_list_",panel,"_",i))) %>% bind_rows(simulations_summary)
    }
    ...
    if(nrow(signals_nc_new)>0){
        ...
        #write.table(enrichment,paste0("burden_enrichment_p_values_",panel,".txt"),quote=F,row.names=F)
        write.table(enrichment,paste0("burden_enrichment_p_values_coverage_list_",panel,".txt"),quote=F,row.names=F)
    }
}

enrichment <- data.frame()
for(panel in levels(as.factor(panel_data$Panel))){
    eval(parse(text=paste0("enrichment_panel <- vroom(\"burden_enrichment_p_values_coverage_list_",panel,".txt\") %>% mutate(panel=\"",panel,"\")")))
...
for(region_type in levels(as.factor(enrichment$region))){
    ...
    CairoPNG(paste0("/slade/projects/UKBB/DNA_Nexus_500k_WGS/Proteomics_50k_3k/results_summaries/rob_working_area/enrichment/coverage_list/qq_test_burden_",region_type,".png"))
    plot <- ggplot() + 
...
enrichment %>% select(-p_for_qq) %>% 
    ...
    write.table("/slade/projects/UKBB/DNA_Nexus_500k_WGS/Proteomics_50k_3k/results_summaries/rob_working_area/enrichment/coverage_list/enrichment_p_values_all_joined_burden.txt",quote=F,row.names=F)