...
for(panel in c(levels(as.factor(panel_data$Panel)),"all")){
    ...
    for(i in seq(1,100)){
        #simulations_summary <- vroom(Sys.glob(paste0("/slade/projects/UKBB/DNA_Nexus_500k_WGS/Proteomics_50k_3k/results_summaries/rob_working_area/enrichment_sims/grouped/single_variant_enrichment_simulations_",panel,"_",i))) %>% bind_rows(simulations_summary)
        simulations_summary <- vroom(Sys.glob(paste0("/slade/projects/UKBB/DNA_Nexus_500k_WGS/Proteomics_50k_3k/results_summaries/rob_working_area/enrichment_sims/coverage_list/single_variant_enrichment_simulations_list_",panel,"_",i))) %>% bind_rows(simulations_summary)
    }
    if(nrow(simulations_summary)>0){
        signals_nc_new <- signals_nc %>% 
            filter(protein_name %in% proteins$Assay)
        signals_nc_ranges <- signals_nc_new %$% 
            GRanges(paste0("chr",CHR),IRanges(start=GENPOS,end=GENPOS))
        ...
        signals_summary <- signals_nc_new %>% 
            summarise(sim_num="results",across(names(signals_nc_new)[4:ncol(signals_nc_new)],~sum(.x)))
        ...
        #write.table(enrichment,paste0("enrichment_p_values_",panel,".txt"),quote=F,row.names=F)
        write.table(enrichment,paste0("enrichment_p_values_coverage_list_",panel,".txt"),quote=F,row.names=F)
    }
}

enrichment <- data.frame()
for(panel in levels(as.factor(panel_data$Panel))){
    eval(parse(text=paste0("enrichment_panel <- vroom(\"enrichment_p_values_coverage_list_",panel,".txt\") %>% mutate(panel=\"",panel,"\")")))
...
for(region_type in levels(as.factor(enrichment$region))){
    ...
    CairoPNG(paste0("/slade/projects/UKBB/DNA_Nexus_500k_WGS/Proteomics_50k_3k/results_summaries/rob_working_area/enrichment/coverage_list/qq_test_",region_type,".png"))
    plot <- ggplot() + 
...
enrichment %>% select(-p_for_qq) %>% 
    ...
    write.table("/slade/projects/UKBB/DNA_Nexus_500k_WGS/Proteomics_50k_3k/results_summaries/rob_working_area/enrichment/coverage_list/enrichment_p_values_all_joined.txt",quote=F,row.names=F)